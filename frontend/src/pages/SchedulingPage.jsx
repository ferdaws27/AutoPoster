...
export default function SchedulingPage() {
  ...
  const handleSchedulePost = async () => {
    console.log('=== HANDLE SCHEDULE POST START ===');
    ...
    if (!newPost.date || !newPost.time) {
      console.log('ERROR: No date or time provided');
    ...
    if (selectedPlatformsList.length === 0) {
      console.log('ERROR: No platforms selected');
    ...
    try {
      console.log('Starting createPost calls...');
      
      // Create scheduled posts for each selected platform using the hook
      const scheduledPosts = await Promise.all(
        selectedPlatformsList.map((platform, index) => {
          console.log(`Creating post ${index + 1} for platform: ${platform}`);
          ...
          console.log(`Post data for ${platform}:`, postData);
          
          return createPost(postData);
        })
      );

      console.log('All posts created successfully:', scheduledPosts);
      ...
      console.log('=== HANDLE SCHEDULE POST SUCCESS ===');
    } catch (error) {
      console.error('=== HANDLE SCHEDULE POST ERROR ===');
  ...
  const handleSyncAll = async () => {
    ...
    try {
      ...
      setSuccessMessage({ 
        ...
        description: `Synchronized ${stats.total} posts from all platforms` 
      });
      setShowSuccessToast(true);
      setTimeout(() => setShowSuccessToast(false), 3000);
                    ...
                    <div key={index} className={`bg-black/10 p-3 relative ${viewMode === 'week' ? 'min-h-[120px]' : 'min-h-[400px]'} ${isToday ? 'border-2 border-cyan-400/20 rounded-lg' : ''}`}>
                      ...
                      <div className="space-y-2">
                        {dayPosts.map((post, postIndex) => (
                          <div 
                            key={post.id}
                            className={`event-bar ${post.platforms && Object.keys(post.platforms).find(p => post.platforms[p]) ? `${post.platforms && Object.keys(post.platforms).find(p => post.platforms[p])?.toLowerCase()}-event` : 'twitter-event'} animate-slide-in`}
