...
export default function HookGeneratorPage() {
  ...
  const generateHooks = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/api/hook-generator/test-generate`, {
      ...
      console.log("Hook generation response:", data);
  ...
  const regenerateHook = async (index) => {
    ...
    const platform = hook.platform?.toLowerCase() || 'twitter';
    ...
    try {
      ...
      // Call API to regenerate a single hook
      ...
      if (response.ok) {
        ...
        if (data.hooks && data.hooks.length > 0) {
          ...
          // Get the first generated hook and adapt it

          const regeneratedHook = data.hooks[0];
    ...
    // Fallback to random hooks if API fails
  ...
  const regenerateAll = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log("Regenerate all response:", data);
      ...
      console.error("Error regenerating all hooks:", error);
...
>
  ...
  <i className="fa-solid fa-pen mr-1"></i> Use
                      ...
                      onClick={(e) => { e.stopPropagation(); regenerateHook(idx); }} 

                      className="p-2 bg-black/30 border border-gray-600 rounded-xl text-gray-300 hover:text-white hover:border-cyan-400 disabled:opacity-50"

                      disabled={regeneratingIndex === idx}

                    >

                      <i className={`fa-solid fa-refresh ${regeneratingIndex === idx ? 'animate-spin' : ''}`}></i>
